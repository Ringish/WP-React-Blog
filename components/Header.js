...
export default class Header extends Component {
		...
		if (sliderImgs) {
			...
			if (sliderImgs.length > 0) {
				setInterval(function() {
					...
					document.getElementById("main-header").style.backgroundImage = 'url("'+sliderImgs[index]+'")';
					index++;
				},10000);
			}
		}
	...
	render() {
		...
		return (
			...
			<header id="main-header" style={this.slider(sliderImgs)} className="main-header">
			<header className="inner-header">
			<h1><NavLink className="logo" to="/">{siteInfo && siteInfo.name}</NavLink></h1>
			</header>
