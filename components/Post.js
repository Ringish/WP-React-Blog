...
const Post = ({ post, selectedBadge, badges }) => (
	<article
		dataTest={get_badge(selectedBadge, badges)}
		dataSelected={selectedBadge}
		dataBadge={post.badge}
		className={
			"post " + showorhide(post.badge, selectedBadge) + " post-" + post.id
		}
	>
		<div className="img-holder">
			{post._embedded["wp:featuredmedia"] && (
				<NavLink
					to={
						post.link.replace("http://sfsu.nu/foreningsdemokrati/", "") ===
						"SHOW_ALL"
							? "/"
							: `/${post.link.replace(
									"http://sfsu.nu/foreningsdemokrati/",
									""
							  )}`
					}
				>
